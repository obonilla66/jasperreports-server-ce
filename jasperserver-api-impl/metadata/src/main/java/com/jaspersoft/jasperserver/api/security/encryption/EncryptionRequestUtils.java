...
public class EncryptionRequestUtils {
    ...
    public static String getValueWithLegacySupport(ServletRequest request, String keyName) {
        ...
        if (value==null && keyName.equals(UsernamePasswordAuthenticationFilterWarningWrapper.SPRING_SECURITY_FORM_PASSWORD_KEY)) {
            value = getValue(request,UsernamePasswordAuthenticationFilterWarningWrapper.LEGACY_SPRING_SECURITY_FORM_PASSWORD_KEY);
        }
        if (value==null && keyName.equals(UsernamePasswordAuthenticationFilterWarningWrapper.SPRING_SECURITY_FORM_USERNAME_KEY)) {
            value = getValue(request,UsernamePasswordAuthenticationFilterWarningWrapper.LEGACY_SPRING_SECURITY_FORM_USERNAME_KEY);
        }
