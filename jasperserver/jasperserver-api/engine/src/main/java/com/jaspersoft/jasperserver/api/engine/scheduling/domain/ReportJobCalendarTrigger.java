  ...
  public ReportJobCalendarTrigger(ReportJobCalendarTrigger jobTrigger) {
	...
	public void copyFromModel(ReportJobTrigger trigger) {
		...
		if (calendarTrigger.isStartTypeModified()) {
			byte startType = calendarTrigger.getStartType();
			setStartType(startType);
			if (startType == START_TYPE_NOW) {
				setStartDate(null);
				setEndDate(null);
			} else {
				if (calendarTrigger.isStartDateModified()) setStartDate(calendarTrigger.getStartDate());
				if (calendarTrigger.isEndDateModified()) setEndDate(calendarTrigger.getEndDate());
			}
		}
		...
		if (calendarTrigger.isWeekDaysModified()) setWeekDays(calendarTrigger.getWeekDays());
