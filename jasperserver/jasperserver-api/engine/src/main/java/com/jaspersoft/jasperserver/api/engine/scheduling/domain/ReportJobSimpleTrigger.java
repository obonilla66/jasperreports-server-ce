...
public class ReportJobSimpleTrigger extends ReportJobTrigger implements Serializable {
  ...
  */
  ...
  public ReportJobSimpleTrigger(ReportJobSimpleTrigger jobTrigger) {
	...
	public void copyFromModel(ReportJobTrigger triggerModel) {
		ReportJobSimpleTriggerModel simpleTriggerModel = (ReportJobSimpleTriggerModel) triggerModel;
		if (simpleTriggerModel.isCalendarNameModified()) setCalendarName(triggerModel.getCalendarName());
		if (simpleTriggerModel.isTimezoneModified()) setTimezone(triggerModel.getTimezone());
		if (simpleTriggerModel.isStartTypeModified()) {
			byte startType = triggerModel.getStartType();
			setStartType(startType);
			if (startType == START_TYPE_NOW) {
				setStartDate(null);
				setEndDate(null);
			} else {
				if (simpleTriggerModel.isStartDateModified()) setStartDate(triggerModel.getStartDate());
				if (simpleTriggerModel.isEndDateModified()) setEndDate(triggerModel.getEndDate());
			}
		}
		if (simpleTriggerModel.isOccurrenceCountModified()) setOccurrenceCount(simpleTriggerModel.getOccurrenceCount());
		if (simpleTriggerModel.isRecurrenceIntervalModified()) setRecurrenceInterval(simpleTriggerModel.getRecurrenceInterval());
		if (simpleTriggerModel.isRecurrenceIntervalUnitModified()) setRecurrenceIntervalUnit(simpleTriggerModel.getRecurrenceIntervalUnit());
		if (simpleTriggerModel.isMisfireInstructionModified()) setMisfireInstruction(simpleTriggerModel.getMisfireInstruction());
	}
