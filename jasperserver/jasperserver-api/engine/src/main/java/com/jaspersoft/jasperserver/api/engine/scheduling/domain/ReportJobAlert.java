...
public class ReportJobAlert implements Serializable {
	...
	public void copyFrom(ReportJobAlertModel alert) {
		if (alert.isRecipientModified()) setRecipient(alert.getRecipient());
		if (alert.isToAddressesModified()) {
			List<String> toAddresses = alert.getToAddresses();
			List<String> copiedToAddresses = new ArrayList<String>(toAddresses.size());
			for (String toAddress : toAddresses) {
				copiedToAddresses.add(toAddress);
			}
			setToAddresses(copiedToAddresses);
		}
		if (alert.isJobStateModified()) setJobState(alert.getJobState());
