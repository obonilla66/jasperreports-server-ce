...
import static com.jaspersoft.jasperserver.inputcontrols.cascade.handlers.InputControlHandler.NULL_SUBSTITUTION_LABEL;
...
import static com.jaspersoft.jasperserver.inputcontrols.cascade.handlers.ParametersHelper.DATE;
import static com.jaspersoft.jasperserver.inputcontrols.cascade.handlers.ParametersHelper.NUMBER;
import static com.jaspersoft.jasperserver.inputcontrols.cascade.handlers.ParametersHelper.array;
import static com.jaspersoft.jasperserver.inputcontrols.cascade.handlers.ParametersHelper.list;
import static com.jaspersoft.jasperserver.inputcontrols.cascade.handlers.ParametersHelper.nullList;
...
public class MultiSelectQueryInputControlHandlerTest {
    ...
    public void convertParameterValueFromRawDataDontDetectNullSubstitutionLabel() throws CascadeResourceNotFoundException, InputControlValidationException {
        ...
        String[] rawData = new String[] {NULL_SUBSTITUTION_LABEL, "Mexico"};
        Object defaultValue = null;
        Object expectedValue = list(NULL_SUBSTITUTION_LABEL, "Mexico");
    ...
    public void convertParameterValueFromRawDataDateParameterWithNull() throws CascadeResourceNotFoundException, InputControlValidationException {
        ...
        gregorianCalendar.setTimeInMillis(858409200000L); //gregorianCalendar.set(1997, GregorianCalendar.MARCH, 15);
        Date date1 = gregorianCalendar.getTime();
        gregorianCalendar.setTimeInMillis(1352098800000L); //gregorianCalendar.set(2012, GregorianCalendar.NOVEMBER, 5);
        ...
        assertThat(expectedValue, is(convertedValue));
    }
    ...
    public void getValue1() throws CascadeResourceNotFoundException {
        ...
        InputControlState envelope = getHandler().getState(ph.getInputControl(CASCADE_COUNTRY), dataSource,
    ...
    public void getValue38() throws CascadeResourceNotFoundException {
        ...
        InputControlState envelope = getHandler().getState(ph.getInputControl(CASCADE_STATE), dataSource,
  ...
  =  * 39 Empty selection for slave control             | default is in query results                      | not mandatory = select none
    ...
    public void formatValueString() throws CascadeResourceNotFoundException {
        ...
        assertThat(array("USA", "Ukraine", "Canada"), is(formattedValues));
    }
    ...
    public void formatValueDouble() throws CascadeResourceNotFoundException {
        ...
        assertThat(array("10" + separator + "1", "10" + separator + "2", "10" + separator + "1234567"), is(formattedValues));
    }
    ...
    public void formatEmptyList() throws CascadeResourceNotFoundException {
        ...
        assertThat(array(), is(formattedValues));
    }
    ...
    public void formatListWithNull() throws CascadeResourceNotFoundException {
        ...
        assertThat(array(NULL_SUBSTITUTION_VALUE), is(formattedValues));
    }
    ...
    public void formatListNonCollection() throws CascadeResourceNotFoundException {
        ...
        try {
            ...
            assertEquals(typedValue, formattedValues[0]);
    ...
    private void mockExecuteQuery() throws CascadeResourceNotFoundException {
        ...
        doAnswer(invocationOnMock -> {
            ...
            String controlName = (String) args[7];
            return EngineServiceCascadeTestQueryExecutor.executeOrderedMap(controlName, (Map<String, Object>) args[5]);
        }).when(cachedEngineService).executeQuery(
                any(ExecutionContext.class), any(ResourceReference.class), anyString(), any(),
                any(ResourceReference.class), anyMap(), anyMap(), anyString()
        );
    }

    private void mockExecuteQueryBean() throws Exception {
        ApplicationContext context = setUpApplicationContext(null, "classpath:/com/jaspersoft/jasperserver/inputcontrols/cascade/handlers/multiSelectQueryInputControlHandler-fixtures-getValue123.xml");
        Mockito.reset(cachedEngineService);
        CascadeTestHelper.setUpCachedEngineService(cachedEngineService, context, "getValue123");
    }

    private void setUpCachedRepositoryService(final CachedRepositoryService cachedRepositoryService) throws CascadeResourceNotFoundException {
        doAnswer(invocationOnMock -> {
            Object[] args = invocationOnMock.getArguments();
