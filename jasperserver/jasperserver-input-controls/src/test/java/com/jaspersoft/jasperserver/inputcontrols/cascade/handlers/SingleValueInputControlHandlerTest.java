...
public class SingleValueInputControlHandlerTest {
    ...
    public void formatValueDate() throws CascadeResourceNotFoundException {
        ...
        assertEquals(1, formattedValues.length);
        assertEquals("2011-07-28", formattedValues[0]);
    }
    ...
    public void formatValueDateTime() throws CascadeResourceNotFoundException {
        ...
        assertEquals(1, formattedValues.length);
        assertEquals("2011-07-28 18:22", formattedValues[0]);
    }
    ...
    public void formatValueTime() throws CascadeResourceNotFoundException {
        ...
        assertEquals(1, formattedValues.length);
        assertEquals("18:22", formattedValues[0]);
    }
    ...
    private void mockMessageSource() {
        doAnswer(invocationOnMock -> {
            ...
            if ("date.format".equals(args[0])) {
                return "yyyy-MM-dd";
            } else if ("datetime.format".equals(args[0])) {
                return "yyyy-MM-dd HH:mm";
            } else if ("time.format".equals(args[0])) {
                return "HH:mm";
            }
            return args[0];
    ...
    private void mockGetDataType() throws CascadeResourceNotFoundException {
        doAnswer(invocationOnMock -> {
            Object[] args = invocationOnMock.getArguments();
            if (DataType.class.isAssignableFrom((Class) args[0])) {
                return (DataType) ((ResourceReference) args[1]).getLocalResource();
            }
            return null;
    ...
    private void mockCalendarFormatProvider() {
        ...
        ((MessagesCalendarFormatProvider) calendarFormatProvider).setMessages(messageSource);
    }
}
