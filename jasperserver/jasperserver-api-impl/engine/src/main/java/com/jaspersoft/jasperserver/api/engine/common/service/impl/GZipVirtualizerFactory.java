...
public class GZipVirtualizerFactory extends AbstractIndividualVirtualizerFactory {
	...
	public GZipVirtualizerFactory() {
		// default values
		maxSize = DEFAULT_MAX_SIZE;
	}
	
	public JRVirtualizer getVirtualizer() {
		JRGzipVirtualizer virtualizer = new JRGzipVirtualizer(maxSize);
		
		if (log.isDebugEnabled()) {
			log.debug("Created GZip virtualizer " + virtualizer 
					+ " using " + maxSize + " as maxSize");
		}
