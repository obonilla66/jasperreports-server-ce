 ...
 * @see
 */
public class DataSourceAccessDeniedException extends ErrorDescriptorException {
    private static final String ERROR_CODE  = "domain.data.source.access.denied";

    public DataSourceAccessDeniedException(ClientReferenceable datasource){
        final ErrorDescriptor errorDescriptor = this.getErrorDescriptor();
        errorDescriptor.setErrorCode(ERROR_CODE)
                .setMessage("Access to datasource is denied.")
