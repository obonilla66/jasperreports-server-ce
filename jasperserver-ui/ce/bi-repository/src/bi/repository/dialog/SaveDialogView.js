...
import RepositoryResourceBundle from '../../../i18n/RepositoryResourceBundle.properties';
...
import dialogTemplate from './resourceChooser/template/saveDialogTemplate.htm';
import errorHandling from '../util/errorHandling';
import Notification from 'js-sdk/src/common/component/notification/Notification';

var mapXhrErrorToMessage = errorHandling.mapXhrErrorToMessage;
...
export default DialogWithModelInputValidation.extend({
    constructor: function (options) {
        ...
        var additionalCssClasses = options.additionalCssClasses || '', title = options.title || RepositoryResourceBundle['dialog.saveDialog.title'], label = options.label || RepositoryResourceBundle['dialog.saveDialog.label'], description = options.description || RepositoryResourceBundle['dialog.saveDialog.description'];
        this.showNotification = options.showNotification || true;
        this.notificationSuccessMessage = options.notificationSuccessMessage || RepositoryResourceBundle['dialog.saveDialog.notification.successMessage'];
        this.confirmationDialogTitle = options.confirmationDialogTitle || RepositoryResourceBundle['dialog.saveDialog.confirm.title'];
        this.confirmationDialogText = options.confirmationDialogText || RepositoryResourceBundle['dialog.saveDialog.confirm.conflict'];
    ...
    initialize: function (options) {
        var self = this;
        DialogWithModelInputValidation.prototype.initialize.apply(this, arguments);    //This is read only model
        ...
        this.foldersTree = repositoryTreeFactory({
            ...
            tooltipOptions: { attachTo: this.$el }
        });
        this.notification = new Notification();
        this.listenTo(this.foldersTree, 'selection:change', function (selection) {
            var parentFolderUri, selectionKey = Object.keys(selection)[0];
    ...
    _showNotification: function (options) {
        var messageType = options.type || 'success', message = options.message;
    ...
    _showCommonErrorMessageOnDialog: function (message) {
        var errorHolder = this.$('[name=commonErrorMessage]');
    ...
    _findNameOfTheLabel: function (resourcesOnServer) {
        var labelWeAreLookingFor = this.model.get('label'), resourceWithTheSameLabel;
    ...
    _checkIfLabelExistsOnServer: function () {
        ...
        return this.model.checkLabelExistenceOnServer().done(this._checkIfLabelExistsOnServerDone.bind(this)).fail(this._checkIfLabelExistsOnServerFail.bind(this));
    },
    ...
    _saveModelOnServer: function (options) {
        ...
        if (options.createNewResource === true) {
            ...
            this._clonedModelForSaving.set({ version: '' }, { silent: true });
    ...
    _saveModelOnServerDone: function (model, data) {
        ...
        this._showNotification({ message: this.notificationSuccessMessage });
        this.trigger('save', this, data);
        this._closeDialog();
    },
    _saveModelOnServerFail: function (model, xhr, options) {
        this._clonedModelForSaving = undefined;    // be sure the dialog is opened to show error to user on dialog
