...
import Model from 'src/components/scalableList/model/ScalableListModel';
import $ from 'jquery';
...
describe('ScalableListModel', function () {
    ...
    beforeEach(function () {
        ...
        model = new Model({ getData: getDataSpy });
    });
    afterEach(function () {
    });
    it('should call get data with correct limit and offset in case if top and bottom equals to buffer size', function () {
        ...
        sinon.assert.calledWith(getDataSpy, {
            offset: 0,
            limit: 100
        });
    });
    ...
    it('should not trust getData\'s total value', function () {
        ...
        model = new Model({ getData: getData });
        model.fetch();
    ...
    it('should recover bufferEndIndex during fetch if it was less than bufferSize because of previous requests', function () {
        var defaultGetData = listGetDataFactory(), returnZero = true, getData = function (options) {
            if (returnZero) {
                var deferred = new $.Deferred();
                deferred.resolve({
                    data: [],
