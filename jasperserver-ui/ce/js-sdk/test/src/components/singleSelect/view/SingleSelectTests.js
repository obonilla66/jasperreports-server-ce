...
import $ from 'jquery';
import SingleSelect from 'src/components/singleSelect/view/SingleSelect';
...
describe("SingleSelect", function () {
    ...
    xit("should add expanded class to the component, scroll to selectied element, focus on input on click on collapsed component", function(){
        ...
        expect($scalableList.scrollTop() > 127500).toBeTruthy();
    ...
    // TODO: Does not working because css file is not applied
    // eslint-disable-next-line no-undef
    xit("should expand component up when it on bottom of page", function() {

        singleSelect.$el.css("margin-top", "400px");

        //Expand
        _expand();

        expect($("body").find(".jr-mSingleselect-dropdown").hasClass("jr-mSingleselect-dropdownTop")).toBeTruthy();
        expect(singleSelect.$el.find(".jr-mSingleselect").hasClass("jr-mSingleselectTop")).toBeTruthy();
    ...
    it("should filter out values on keydown in search input", function(){
        ...
        expect($scalableList.find("li").length).toEqual(9);
    });
    ...
    // TODO: Does not working because css file is not applied
    // eslint-disable-next-line no-undef
    xit("should scroll to current component's selected element if it's present in result set after filtering out", function(){
        _setValue("40");
        ...
        //type value in search
        singleSelect.$el.find("input").val("4");
        var keyDownEvt = $.Event("keydown");
        singleSelect.$el.find("input").trigger(keyDownEvt);

        expect($scalableList.find("li[data-index='4']").hasClass("jr-isSelected")).toBe(true);
        var selectedItemTop = $scalableList.find("li[data-index='39']").offset().top;
    ...
    it("should select an item in component when keyDown is pressed and value was set through API for single element for same value using object notation", function() {
        _setValue({"0":1});
        expect(singleSelect.model.get("value").label).toBeUndefined();
        _setValue({
            "0": {
                value: 1,
                label: 'Jun'
        ...
        expect($scalableList.find("li[data-index='1']").hasClass("jr-isSelected")).toBe(true);
        expect(singleSelect.model.get("value").label).toBe("Jun");
    });
