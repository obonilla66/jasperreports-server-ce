...
import static com.jaspersoft.jasperserver.api.logging.audit.domain.AuditEventType.OLAP_VIEW;
...
public class OlapModelController extends JRBaseMultiActionController {
    ...
    private void createAuditEvent(final String olapUnitUri) {
        auditContext.doInAuditContext(new AuditContext.AuditContextCallback() {
            public void execute() {
                AuditEvent auditEvent = auditContext.getOpenedAuditEvent(OLAP_VIEW.toString());
                if (auditEvent == null) {
                    auditEvent = auditContext.createAuditEvent(OLAP_VIEW.toString(), olapUnitUri, null);
                } else {
    ...
    private void updateAuditEvent(final String olapUnitUri) {
        auditContext.doInAuditContext(OLAP_VIEW.toString(), new AuditContext.AuditContextCallbackWithEvent() {
